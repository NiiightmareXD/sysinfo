...
/// Converts the value into a parallel iterator (if the multi-thread feature is
/// enabled). Uses the `rayon::iter::IntoParallelIterator` trait.
#[cfg(all(
    all(
        any(
            target_os = "linux",
...
/// Converts the value into a sequential iterator (if the multithread feature is
